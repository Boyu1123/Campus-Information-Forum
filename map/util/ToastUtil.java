...
public class ToastUtil {
	...
	public static void showerror(Context context, int rCode){
		...
		try {
	        ...
	        default:
//	        	Toast.makeText(context,"鏌ヨ澶辫触锛�+rCode , Toast.LENGTH_LONG).show();
                logError("鏌ヨ澶辫触", rCode);
                break;
	        }
        } catch (Exception e) {
    ...
    private static void logError(String info, int errorCode) {
        ...
        print(LINE);//title
        print(info);
        print("閿欒鐮� " + errorCode);
        print("                                                                               ");
        print("濡傛灉闇�鏇村淇℃伅锛岃鏍规嵁閿欒鐮佸埌浠ヤ笅鍦板潃杩涜鏌ヨ");
        print("  http://lbs.amap.com/api/android-sdk/guide/map-tools/error-code/");
        print("濡傝嫢浠嶆棤娉曡В鍐抽棶棰橈紝璇峰皢鍏ㄩ儴log淇℃伅鎻愪氦鍒板伐鍗曠郴缁燂紝澶氳阿鍚堜綔");
        print(LINE);//end
    }
